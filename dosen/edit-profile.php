...
$queryLecturer = mysqli_query($conn, "SELECT * FROM lecturer WHERE nip = '{$_SESSION["nip"]}'");
  ...
  <div class="bg-[#f9fafb] p-6 sm:ml-64 h-screen">
    ...
    <div class="bg-white rounded-lg shadow-[0_0_0_1px_rgba(0,0,0,0.05)] p-4">
      <form action="./controller/update_profile.php" method="POST" class="grid grid-cols-2 gap-x-12 p-4 w-[60rem]" enctype="multipart/form-data">
        <input type="hidden" name="nip" value="<?= $lecturer["nip"] ?>" />
        <input type="hidden" name="oldImage" value="<?= $lecturer["image"] ?>" />
        <div>
          ...
          <div class="mb-3.5">
            <div class="relative h-[50px]">
              <input type="number" name="nip" id="nip" class="form__input" autocomplete="off" placeholder=" " value="<?= $lecturer["nip"] ?>" disabled />
              <label for="nip" class="form__label">NIP</label>
            </div>
          </div>
          <div class="mb-3.5">
            <div class="relative h-[50px]">
              <input type="text" name="name" id="name" class="form__input" autocomplete="off" placeholder=" " value="<?= $lecturer["name"] ?>" required />
              <label for="name" class="form__label">Nama Lengkap</label>
            </div>
          </div>
          <div class="relative mb-3 h-[50px]">
            <select name="position" class="form__input dropdown" required>
              <option disabled selected>-- Pilih Jabatan --</option>
              <option <?= $lecturer["position"] == "Asisten Ahli" ? "selected" : "" ?>>Asisten Ahli</option>
              <option <?= $lecturer["position"] == "Lektor" ? "selected" : "" ?>>Lektor</option>
              <option <?= $lecturer["position"] == "Lektor Kepala" ? "selected" : "" ?>>Lektor Kepala</option>
              <option <?= $lecturer["position"] == "Guru Besar" ? "selected" : "" ?>>Guru Besar</option>
            </select>
            <label class="form__label" for="dropdown"></label>
          </div>
          <div class="mb-6">
            <div class="relative h-[50px]">
              <input type="text" name="phone_number" id="phone_number" class="form__input" autocomplete="off" placeholder=" " value="<?= $lecturer["phone_number"] ?>" required />
        ...
        <div>
          ...
          <div class="mb-3.5">
            <div class="relative h-[50px]">
              <input type="email" name="email" id="email" class="form__input" autocomplete="off" placeholder=" " value="<?= $lecturer["email"] ?>" onblur="validateInput('email')" required />
          ...
          <div class="">
            ...
            <div class="flex justify-between items-center gap-x-6">
              <img src="../img/<?= $lecturer["image"] ?>" alt="" id="image-preview" class="w-20 h-20 rounded-full object-cover">
              <div>
